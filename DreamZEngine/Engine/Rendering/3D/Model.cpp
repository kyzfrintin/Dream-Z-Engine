...
Model::Model(Shader* ourShader_, string const &path, bool gamma) : gammaCorrection(gamma)
{
	loadModel(path);
...
void Model::UpdateInstance(GLuint index_, glm::vec3  position_, float angle_, glm::vec3 rotation, glm::vec3 scale_)
{
...
void Model::Render()
{
	glm::mat4 projection = glm::perspective(glm::radians(Camera::GetInstance()->Zoom), (float)800 / (float)600, 0.1f, 100.0f);
	...
	for (GLuint i = 0; i < meshes.size(); i++)
		meshes[i].Render(ourShader, modelInstances);
}
...
void Model::loadModel(string const &path)
{
	...
	if (scene == NULL || scene->mAnimations == 0) // if is Not Zero
	{
		cout << "ERROR::ASSIMP:: file has no animationi" << importer.GetErrorString() << endl;
		//return;
	}
